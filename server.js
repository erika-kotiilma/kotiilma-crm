...
{"category":"hot tai warm tai cold","score":1-10,"reasoning":"1-2 lausetta suomeksi MIKSI tämä kategoria ja mainitse tärkein syy lomakkeen vastauksista","sms_text":"personoitu SMS suomeksi max 160 merkkiä mainitse etunimi ja konkreettinen asia lomakkeesta","followup_text":"seurantaviesti 24h päästä suomeksi max 160 merkkiä"}`;
  ...
  if (pathname === '/webhook/meta') {
    ...
    if (req.method === 'POST') {
      ...
      // Helper: lukee field_data arraystä arvon nimen perusteella
      function getField(fieldData, fieldName) {
      ...
      // Tukee sekä Facebookin natiivia field_data arrayta että Maken purkamat kentät
      const fd = body.field_data || [];
      const name = body.name || getField(fd, 'full_name') || '';
      const phone = body.phone || getField(fd, 'phone_number') || '';
      const email = body.email || getField(fd, 'email') || '';
      const address = body.address || body.city || getField(fd, 'street_address') || '';
      const zip = body.zip || getField(fd, 'zip_code') || '';
      ...
      const lead = {
        ...
        city: address,
        zip,
        source: body.source || body.ad_name || 'Meta',
        install_timing: body.install_timing || getField(fd, 'milloin_asennus_olisi_ajankohtainen') || '',
        property_type: body.property_type || getField(fd, 'kiinteistön_tyyppi') || '',
        property_size: body.property_size || getField(fd, 'kiinteistön_koko') || '',
